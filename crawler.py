...
def recent_publish_time():
	page_soup = open_url_to_soup(raw_url)
	recent_article = page_soup.find("article", {"class":"article"})
	local_publish_time = recent_article.time.get('datetime')
	publish_date = local_publish_time.split("T")[0]
	
	return publish_date


def date_push_compare():
	utc = datetime.utcnow()
	utc.isoformat(timespec='seconds')
	jp_now = utc + timedelta(hours = 9)

	publish_date = recent_publish_time()
	jp_now_date = str(jp_now).split(" ")[0]

	if jp_now_date == publish_date:
		return True
	else:
		return False
